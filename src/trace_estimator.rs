...
pub fn estimate_tr_ki_kj(
    ...
    let gj_z_col_sum = {
        ...
        gj_z.axis_iter(Axis(1))
            .into_par_iter()
            .map(|col| sum_f32(col.iter()))
            .collect_into_vec(&mut col_sums);
...
pub fn estimate_tr_k_gxg_k(
    ...
    //      .collect_into_vec(&mut sums);
    //    (sums.into_iter().sum::<f32>() / (geno_arr.dim().1 *
...
pub fn estimate_gxg_gram_trace(
    ...
    squashed
        .axis_iter(Axis(1))
        .into_par_iter()
        .map(|uugg| sum_of_squares(uugg.iter()))
        .collect_into_vec(&mut sums);
...
pub fn estimate_gxg_kk_trace(
    ...
    //        let rand_vecs = generate_plus_minus_one_bernoulli_matrix(num_cols,
    // num_rand_z_vecs);        let geno_arr_dot_rand_vecs =
    // geno_arr.dot(&rand_vecs);        let ggz =
    // wg.dot(&geno_arr_dot_rand_vecs);        sum += (((&ggz * &ggz).sum()
    // / num_rand_z_vecs as f32 - s) / 2.) as f64;    }
