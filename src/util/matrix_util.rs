...
mod tests {
    ...
    fn test_normalize_vector_inplace() {
        ...
        assert!(
            mean(vec.iter()).abs() > 1e-3,
            "the randomly generated vector should have a large non-zero mean"
        );
        assert!(
            (standard_deviation(vec.iter(), ddof) - 1.).abs() > 2.,
            "the randomly generated vector should have a large std"
        );
