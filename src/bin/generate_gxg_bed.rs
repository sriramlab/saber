...
fn main() {
    ...
    let chunk_size = 100;
    let total_chunks =
        num_g_snps / chunk_size + (num_g_snps % chunk_size > 0) as usize;
    bed.col_chunk_iter(chunk_size, None).enumerate().for_each(
        |(chunk_index_i, chunk_i)| {
            println!(
                "processing chunk [{}/{}]",
                chunk_index_i + 1,
                total_chunks
            ...
            .for_each(|(chunk_index_j, chunk_j)| {
                ...
                for (i, col_i) in chunk_i.gencolumns().into_iter().enumerate() {
                    for (j, col_j) in
                        chunk_j.gencolumns().into_iter().enumerate()
                    {
                        if same_chunk && j <= i {
                            continue;
                        }
                        ...
                        for _ in 0..num_people / 4 {
                            buf_writer
                                .write(&[geno_to_lowest_two_bits(
                                    col_i[k] * col_j[k],
                                ) | (geno_to_lowest_two_bits(
                                    col_i[k + 1] * col_j[k + 1],
                                ) << 2)
                                    | (geno_to_lowest_two_bits(
                                        col_i[k + 2] * col_j[k + 2],
