...
use program_flow::{
    argparse::{
        extract_optional_numeric_arg, extract_optional_str_arg, extract_str_arg,
    },
    OrExit,
};
...
use saber::{
    simulation::sim_pheno::{
        ...
        get_sim_output_path, write_effects_to_file, SimEffectMechanism,
    },
    util::{get_bed_bim_fam_path, get_fid_iid_list},
};

fn get_le_snp_counts_and_effect_sizes(
    count_filename: &String,
) -> Result<Vec<(usize, f64)>, String> {
    let buf = match OpenOptions::new().read(true).open(count_filename.as_str())
    {
        Err(why) => {
            return Err(format!("failed to open {}: {}", count_filename, why))
        }
        Ok(f) => BufReader::new(f),
    };
    let count_vec: Vec<(usize, f64)> = buf
        .lines()
        .map(|l| {
            let vec2: Vec<String> = l
                .unwrap()
                .split_whitespace()
                .map(|t| t.to_string())
                .collect();
            (
                vec2[0].parse::<usize>().unwrap(),
                vec2[1].parse::<f64>().unwrap(),
            )
        })
        .collect();
    Ok(count_vec)
...
fn main() {
    ...
    if let Some(g_var) = g_var {
        if g_var > 0. {
            ...
            let out_path =
                get_sim_output_path(&out_path_prefix, SimEffectMechanism::G);
    ...
    if let Some(le_snps_bfile) =
        extract_optional_str_arg(&matches, "le_snps_bfile")
    {
        println!("\n=> generating GxG effects");
        let (le_snps_bed_path, le_snps_bim_path, le_snps_fam_path) =
            get_bed_bim_fam_path(&le_snps_bfile);
        let gxg_component_count_filename =
            extract_optional_str_arg(&matches, "gxg_component_count_filename")
                .unwrap_or_exit(Some(
                    "must provide --counts as le_snps_bfile is specified",
                ));
        ...
        let fid_iid_list =
            get_fid_iid_list(&le_snps_fam_path).unwrap_or_exit(None::<String>);
        ...
        let counts_and_effect_sizes =
            get_le_snp_counts_and_effect_sizes(&gxg_component_count_filename)
                .unwrap_or_exit(Some(
                "failed to get GxG component LE SNP counts and effect sizes",
            ));
        let num_gxg_components = counts_and_effect_sizes.len();

        let mut acc = 0usize;
        for (i, (c, effect_size)) in
            counts_and_effect_sizes.into_iter().enumerate()
        {
            println!(
                "GxG component [{}/{}] expects {} LE SNPs",
                i + 1,
                num_gxg_components,
                c
            );
            let out_path = get_sim_output_path(
                &out_path_prefix,
                SimEffectMechanism::GxG(i + 1),
            );
            let gxg_basis = le_snps_arr.slice(s![.., acc..acc + c]).to_owned();
            let effects = generate_gxg_contribution_from_gxg_basis(
                gxg_basis,
                effect_size,
            );
            println!(
                "\n=> writing the effects due to GxG component {} to {}",
                i + 1,
                out_path
            );
            write_effects_to_file(&effects, &fid_iid_list, &out_path)
