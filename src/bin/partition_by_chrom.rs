...
use program_flow::argparse::{extract_str_arg, extract_str_vec_arg};
...
fn main() {
    ...
    println!("PLINK bim path: {:?}\nout_path: {}", bim_path_list, out_path);
    ...
    for path in bim_path_list.iter() {
        ...
        for (i, line) in BufReader::new(file).lines().enumerate() {
            ...
            let partition = toks.next()
                                .unwrap_or_exit(Some(format!("failed to extract the chrom from line {} in {}", i, path)))
                                .to_string();
