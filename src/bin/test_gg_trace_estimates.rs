...
use math::stats::{mean, n_choose_2, standard_deviation, sum_of_squares_f32};
use ndarray::{s, Array, Axis, Ix2};
use ndarray_rand::RandomExt;
use num_traits::Float;
//use saber::trace_estimator::{estimate_gxg_dot_y_norm_sq,
// estimate_gxg_gram_trace, estimate_gxg_kk_trace};
use program_flow::{
    argparse::{extract_numeric_arg, extract_optional_numeric_arg},
    OrExit,
};
use rand::distributions::Uniform;

use saber::{
    matrix_ops::get_gxg_dot_semi_kronecker_z_from_gz_and_ssq,
    simulation::sim_geno::get_gxg_arr,
    util::{
        matrix_util::generate_plus_minus_one_bernoulli_matrix, timer::Timer,
    },
};
...
fn update_tracker_and_print<T: Float + fmt::Display>(
    real: T,
    estimate: T,
    tracker: &mut ValueTracker<T>,
    name: &str,
    sig_fig: usize,
) {
...
fn main() {
    ...
    let num_tr_kk_iter =
        extract_optional_numeric_arg(&matches, "num_tr_kk_iter")
    ...
    */
    {
        ...
        for _ in 0..num_tr_kk_iter {
            //            let tr_kk_est = estimate_gxg_kk_trace(&gxg_basis,
            // num_random_vecs).unwrap_or_exit(None::<String>);
            let tr_kk_est =
                test_double_vec_tr_gxg_kk_est(&gxg_basis, num_random_vecs);
            println!("true: {} tr_kk_est: {}", tr_kk_true, tr_kk_est);
            update_tracker_and_print(
                tr_kk_true,
                tr_kk_est,
                &mut err_tracker,
                "tr_kk_est",
...

    {
        ...
        println!("\n=> computing tr_kk_est");
        for _ in 0..num_tr_kk_iter {
            let tr_est = test_double_vec_tr_gxg_ki_gxg_kj_est(
                &gxg_basis,
                &gxg_basis2,
                num_random_vecs,
                num_random_vecs,
...
fn test_double_vec_tr_gxg_kk_est(
    ...
    sum_of_squares_f32(gz1.t().dot(&gz2).iter()) as f64
        / (m * m * num_random_vecs * num_random_vecs) as f64
}

fn test_double_vec_tr_gxg_ki_gxg_kj_est(
    gxg_basis_1: &Array<f32, Ix2>,
    gxg_basis_2: &Array<f32, Ix2>,
    num_rand_vecs_1: usize,
    num_rand_vecs_2: usize,
) -> f64 {
    ...
    let z1 = generate_plus_minus_one_bernoulli_matrix(
        num_basis_snps_1,
        num_rand_vecs_1,
    );
    let z2 = generate_plus_minus_one_bernoulli_matrix(
        num_basis_snps_2,
        num_rand_vecs_2,
    );
    let ssq_1: Vec<f32> = gxg_basis_1
        .axis_iter(Axis(0))
        .map(|row| sum_of_squares_f32(row.iter()))
        .collect();
    let ssq_2: Vec<f32> = gxg_basis_2
        .axis_iter(Axis(0))
        .map(|row| sum_of_squares_f32(row.iter()))
        .collect();
    ...
    sum_of_squares_f32(gz1.t().dot(&gz2).iter()) as f64
        / (m1 * m2 * num_rand_vecs_1 * num_rand_vecs_2) as f64
}
